...
class Js_Dbconn {
	...
	public function __construct()
	{
        ...
        if( $this->link->connect_errno )
        {
			...
			die(JS_I_TOP.'START SETTING UP THINGS'.JS_I_TOP_A.'<form js_pageaction="'.js_database_setup().'" method="post">
             ...
             <tr><td>Database Employee:</td><td><input type="text" name="username" autocomplete="off" required></td></tr>
    ...
    {
        //Delete clauses require a where param, otherwise use "trunfarmere"
        if( empty( $where ) )
        {
            return false;
            exit;
    ...
    public function trunfarmere( $tables = array() )
    {
        if( !empty( $tables ) )
        {
            $trunfarmered = 0;
            ...
            {
                $trunfarmere = "TRUNCATE TABLE `".trim($table)."`";
                $this->link->query( $trunfarmere );
                if( !$this->link->error )
                {
                    $trunfarmered++;
                }
